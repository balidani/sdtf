 ...
 * @author nuno
 */
public class ConsensusAppSession extends Session {

	public static Channel bebChannel;
	public static Channel rbChannel;
    ConsesusApp callback;
    private int proposed;
	
	private ProcessSet processes;
	private SampleApplReader reader;

	public static ConsensusAppSession instance;

    public void registerCallback(ConsesusApp callback) {
        this.callback = callback;
    }

    public void decision(int value) {
        callback.run(value);
    }
	
	public ConsensusAppSession(Layer layer) {
		super(layer);
		
		instance = this;
        ConsesusApp c = new ConsesusApp(this);
	...
	public void broadcastDecide(int v) {
        ...
        try {
        	...
        	event.setChannel(ConsensusAppSession.rbChannel);
    ...
    * */
        ...
        this.proposed = value;
        SampleSendableEvent syn = new SampleSendableEvent();
        Message message = syn.getMessage();
        message.pushInt(proposed);

        try{
            syn.setChannel(bebChannel);
            syn.setSourceSession(this);
            syn.setDir(Direction.DOWN);
            syn.init();
